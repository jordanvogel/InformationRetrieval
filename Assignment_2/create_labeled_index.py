    ...
    for f in sorted(listdir(directory), key=lambda x: int(x.split(".")[0])):
        ...
        for word in docText:
            ...
            if word not in index:
                index[word] = {'df':1, 'postings':[(docid,{'tf':1, 'tf-idf':0, 'pos':[pos]}, category)]}
            else:
                ...
                # docid not in posting list
                    ...
                    index[word]['postings'].insert(doc_index[1], (docid, {'tf':1,'tf-idf':0, 'pos':[pos]}, category))
                    index[word]['df'] += 1
            pos += 1
...
if __name__ == "__main__":
    ...
    if len(argv) != 3:
        raise SyntaxError("Expected usage ./create_labeled_index.py [document dir] [index dir]")
