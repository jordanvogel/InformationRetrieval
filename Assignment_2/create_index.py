...
def generate_posting_list(directory):
    ...
    for term in index.keys():
        for docid_index, docid in enumerate(index[term]['postings']):
            tf_idf = float((1 + log10(index[term]['postings'][docid_index][1]['tf'])) * log10(DOC_COUNT/int(index[term]['df'])))
            index[term]['postings'][docid_index][1]['tf-idf'] = tf_idf

    return (index, DOC_COUNT, doclen, docs)
